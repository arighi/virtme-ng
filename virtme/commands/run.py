...
def do_it() -> int:
    ...
    if args.systemd:
        ...
        kernelargs.append(f"console={arch.serial_console_args()[0]}")
        initsh.extend(
            [
                ...
                "mount --bind /dev/null /usr/lib/systemd/system/systemd-journald.service",
            ]
        )

    if args.root == "/":
        if args.systemd:
            initsh.append(f"SYSTEMD_UNIT_PATH={CACHE_DIR}: exec /sbin/init")
